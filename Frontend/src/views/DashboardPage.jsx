  ...
  try {
    const saved = localStorage.getItem(`moora_active_room_${userSession}`);
  ...
  const cleanUpCollaboration = () => {
    ...
    const provider = new YjsWsProvider(
      ...
      {
        onLanguage: (lang) => {
          // Reflects language changes made by the other collaborator
          setLanguage(lang);
        }
      }
    );
    ...
    // now:
  const handleSelectFileNode = async (fileRecord) => {
    if (editorRef.current && fileName) {
      const currentCode = editorRef.current.getValue();
      setFileList(prev => prev.map(f => f.fileName === fileName ? { ...f, codeContent: currentCode } : f));
      await flushFileToBackend(fileName, language, currentCode); // NEW — persist the file we're leaving
  }
    ...
    const handleCloseTab = async(e, tabToClose) => {
      e.stopPropagation(); 
      ...
      if (fileName === tabToClose.fileName) {
        if (updatedTabs.length > 0) {
          if (editorRef.current) {
            await flushFileToBackend(tabToClose.fileName, language, editorRef.current.getValue()); // NEW
          }
    ...
    // add this new function, near triggerBackgroundWorkspaceSave:
    const flushFileToBackend = async (targetFileName, targetLanguage, code) => {
      if (!userSession || !targetFileName) return;
      try {
        await fetch(`${API_BASE}/api/code/save`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        console.error('Failed to flush outgoing file before switching:', err);
      }
    };
        ...
        <header className="h-14 border-b border-[#1b1b24] bg-[#0c0c14] flex items-center justify-between px-4 shrink-0 z-20">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              <button
                onClick={() => navigate(`/live/${activeRoom.roomId}`)}
              ...
              >
                Back to Room{activeRoom.filename ? ` (${activeRoom.filename})` : ''}
              </button>
              <button
                onClick={() => {
                ...
                className="w-9 h-9 flex items-center justify-center rounded-lg text-slate-500 hover:text-white hover:bg-white/5 transition-colors cursor-pointer"
              >
                ✕
              </button>
            </div>
            ...
            <GoLiveButton
              ...
              onBeforeOpen={triggerBackgroundWorkspaceSave}
            />
          )}
                  ...
                  <div
                    ...
                    onClick={async () => {
                      ...
                      if (editorRef.current) {
                        const currentCode = editorRef.current.getValue();
                        setFileList(prev => prev.map(f => f.fileName === fileName ? { ...f, codeContent: currentCode } : f));
                        await flushFileToBackend(fileName, language, currentCode); // NEW
                      }
